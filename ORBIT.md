...
| `maploader/MapLoader.kt` | `MapLoader.resolve(name)` (returns `worlds/<name>.nebula` path, auto-ingests from `maps/` on first use), `scanAndIngest()` (boot-time bulk ingest), `ingestAnvilIfPresent(name)` (copies `maps/<name>/` → `worlds/<name>/` → converts to `worlds/<name>.nebula` → deletes working copy; Anvil source preserved; skips when a valid `.nebula` already exists) |
