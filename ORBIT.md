...
- `PlayerTracker` — `ConcurrentHashMap<UUID, PlayerState>`. Properties: `alive`, `spectating`, `disconnected` (Set<UUID>), `aliveCount`, `size`. Methods: `join`, `eliminate`, `revive`, `disconnect`, `reconnect`, `remove`, `stateOf`, `isAlive`, `isSpectating`, `isDisconnected`, `contains`, `clear`.
