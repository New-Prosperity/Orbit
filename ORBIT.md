...
- Hazelcast: **client mode** with 25 stores (includes ReconnectionStore, HostTicketStore, HostRequestStore, HostRequestLookupStore, BattleRoyaleKitStore, NickStore, NickPoolStore, RatingStore). Smart client auto-discovers all members after initial connection. Near-cache configs wired automatically by HazelcastModule for stores with `nearCacheSeconds > 0`.
...
- **Map loading**: Runtime only loads `.nebula` files — Anvil is not a runtime format. When `gameMode` is set and `STORAGE_URL`/`STORAGE_TOKEN` are provided, loads map via `MapLoader.resolve()` wrapped in `runCatching`; on failure, logs the error and falls back to the season default world path (null `resolvedWorldPath`). `ReplayStorage` initialized with a `"replays"` scope from the same storage client.
- **Anvil ingest — three-directory model**: `maps/` is the archival source (human-managed Anvil drops, never deleted or mutated). `worlds/` is the runtime directory holding `.nebula` artifacts. Flow for `MapLoader.ingestAnvilIfPresent(name)`: (a) recursively copy `maps/<name>/` → `worlds/<name>/` working copy; (b) hand the working copy to `NebulaWorldConverter` (which briefly spins a Minestom instance backed by `AnvilLoader` that may mutate files — hence the copy); (c) write the compressed artifact to `worlds/<name>.nebula` and verify it reads back; (d) recursively delete the `worlds/<name>/` working copy. On failure the working copy is cleaned, the `.nebula` target is removed, `maps/<name>/` stays untouched. If `worlds/<name>.nebula` already exists and is a valid world, conversion is skipped. `MapLoader.resolve(name)` looks in `worlds/` for `.nebula` files (and lazy-ingests from `maps/` on first use). BuildMode `/save` and `/load` also read/write under `worlds/`. To force a re-ingest, delete the `.nebula` in `worlds/` and restart. Staff commands: `/orbit map list` (shows `worlds/*.nebula` + any pending `maps/<name>/` anvil dirs), `/orbit map scan` (re-runs the boot scan), `/orbit map convert <name>` (ingest one).
- Init order: `environment {}` → `appDelegate` → `app.start()` → provision self-discovery → map loading (with fallback) → `MinecraftServer.init()` → `resolveMode()` → ensure `data/models/` directory → `ModelEngine.install()` → `BotAI.install()` → `CustomContentRegistry.init()` (loads items, blocks, armors) → load `.bbmodel` model files → `CustomContentRegistry.mergePack()` (generates armor shaders + HUD shaders + HUD font, merges all) → register HUD test layout → `HudManager.install(handler)` → register commands (basic, game, orbit, model, cc, cinematic, screen, armor, hud, nick) → `mode.install(handler)` → common listeners → HUD tick task (2 ticks) → `server.start()` → registration → shutdown hook.
...
- **Graceful shutdown**: Shutdown hook cancels global tasks, calls `BotAI.uninstall()`, `NickManager.uninstallListeners()`, `VanishManager.uninstallListeners()`, `StaffSpectateManager.uninstallListeners()`, kicks all connected players ("Server shutting down") before deregistering, so Velocity redirects them to a hub instead of disconnecting.
- **Resource pack**: Pack is merged at startup but NOT sent from Orbit — pack distribution is delegated to the proxy.
- Common listeners: `AsyncPlayerConfigurationEvent` (set spawning instance/respawn from mode), `PlayerSpawnEvent` (locale tag via `LOCALE_TAG` on Player entity, trace ID tag via `TRACE_TAG` from `SessionStore`, nick application + nicked-player broadcast to joining viewer), `PlayerDisconnectEvent` (cleanup), `OnlinePlayerCache` refresh (5s).
- **Network message subscribers**: registered via `installNetworkSubscribers(gameMode)` (in `me.nebula.orbit.subscriber/NetworkSubscribers.kt`). Currently handles `PlayerReportMessage` (staff alert fan-out), `PartyInviteMessage` (toast on invitee), `FriendOnlineMessage` (chat + sound to listed friend ids only — looks up by uuid instead of iterating all online), and (when `gameMode != null`) `ConfigUpdateMessage` for `MAINTENANCE_<gameMode>` notices. Locale is stored as `Tag.String("nebula:locale")` on the Player entity — no manual eviction needed, auto-cleans on entity removal. Trace ID stored as `Tag.String("nebula:traceId")` — read from `SessionData.traceId` (set by Nova on login), fallback to first 8 chars of player UUID. Extension property `Player.traceId` provides convenient access. `localeOf(UUID)` resolves locale by looking up the player from the connection manager.
...
- **`/nickpool add <username>`** — permission `nebula.nick.admin`. Fetches skin from Mojang API via `fetchMojangSkin()` using Ether's `httpClient {}` DSL (10s timeout), with `runCatching` error handling (replies "orbit.nick.fetch_failed" on failure), validates name, adds to `NickPoolStore`.
...
- **Queue feedback**: Subscribes to `QueuePositionMessage` — sends action bar with position, total queue size, and estimated wait (seconds) every 3s (QueueProcessor interval). Sound: `BLOCK_NOTE_BLOCK_PLING` (0.5f volume). Unsubscribes on shutdown.
- **Queue provisioning**: Subscribes to `QueueProvisioningMessage` — sends "Server is starting..." action bar to queued players.
- **Party queue notification**: Subscribes to `PartyQueueNotificationMessage` — notifies party members with action bar when their leader queues. Sound: `UI_BUTTON_CLICK`.
- **Queue assignment sounds**: On `QueueAssignmentMessage`, plays `ENTITY_PLAYER_LEVELUP` for all assigned players.
- **Queue removed sounds**: On `QueueRemovedMessage`, plays `BLOCK_NOTE_BLOCK_BASS` for affected player.
- **Game end re-queue**: Subscribes to `GameEndMessage` — shows re-queue prompt with clickable text when player arrives on hub after game.
- **Disconnect cleanup**: `PlayerDisconnectEvent` removes hotbar, clears `HostMenu.removePending(uuid)`, and clears `SelectorMenu.removeAllQueued(uuid)` to prevent stale state.
...
- **Statues**: `StatueManager.install()` on startup, `StatueManager.uninstall()` on shutdown. Supports manual statues via `/statue` command, automatic top-player statues via `installAutoRefresh()`, and programmatic statues via `spawn()`.
- No `MechanicLoader` — hub mode has no mechanics enabled.
- `shutdown()` calls `StatueManager.uninstall()`, cancels tab refresh task, calls `NameplateManager.uninstall()`, unsubscribes host status, queue removed, queue assignment, and queue position listeners; uninstalls scoreboard, tab list, lobby, and hotbar.
...
- **ENDING**: Winners (alive players) assigned placement 1. `MatchResultDisplay.broadcast()`. Publishes `GameEndMessage` before shutdown so hub can show re-queue prompt. `Store.flushAll()` before `app.stop()`. Panel auto-terminates provision on server stop. No `Runtime.halt(0)`.
...
- `PlayerTracker` — `ConcurrentHashMap<UUID, PlayerState>` with full team, lives, and stats tracking.
  - **State properties**: `alive`, `spectating`, `disconnected`, `respawning`, `all` (Set<UUID>). `aliveCount`, `effectiveAliveCount` (non-spectators), `size`. `visibleAliveCount()` — counts alive players excluding vanished (uses `VanishManager.isVanished`), for player-facing displays.
...
`install()` wraps core event listeners (`PlayerSpawnEvent`, `PlayerDisconnectEvent`, `PlayerUseItemEvent`) in a child `EventNode` (`gamemode-core-<name>`) added to the global handler. `shutdown()` removes this node, ensuring clean listener removal (important for GameTest framework cleanup).
...
- `createGameInstance()` — open, override in subclass for custom instance creation (e.g. procedural generation). Base loads from `settings.worldPath` via `NebulaWorldLoader.loadAndPreload()` (path resolved through `MapLoader`, which ingests any Anvil source first).
...
- **Replay recording**: Dual recorder system — `PacketReplayRecorder` (pixel-perfect packets) and `ReplayRecorder` (semantic frames). Both started in `enterPlaying()`. Semantic recorder captures positions every 4 ticks for alive players, deaths via `handleDeath()`, chat via game mechanics node. In `enterEnding()`: stops both, saves packet replay as `.nebr`, saves semantic replay as JSON, runs `ReplayHighlights.detect()` for highlight analysis, publishes `ReplayHighlightMessage` for MULTI_KILL/CLUTCH highlights via `NetworkMessenger`. Replay key: `"${serverName}-${timestamp}"`.
- **Replay command**: `/replay` command with subcommands: `list`, `play`, `stop`, `pause`, `resume`, `speed`, `seek`, `pov`, `highlights`, `info`. Permission `orbit.replay` for playback, `orbit.replay.admin` for list/info. Viewer cleanup on disconnect via `cleanupReplayViewer()`.
- **Highlight detection**: `ReplayHighlights.detect()` analyzes semantic frames for FIRST_BLOOD, FINAL_KILL, MULTI_KILL (3+ kills in 10s), LONG_RANGE_KILL (20+ blocks), CLUTCH (1vN win).
...
- `buildComboCounter(): ComboConfig?` — combo counter configuration, null = none. Installed/uninstalled by base.
...
**First blood**: First kill in a game triggers `orbit.game.first_blood` broadcast to all players. Uses `displayUsername` (nick-aware).

**Nick-aware player names**: All player-visible messages in `GameMode` base (first blood, death recap attacker names, weapon/distance details) use `Player.displayUsername` from `OrbitExtensions.kt`, which returns the nicked name if nicked, or real username otherwise. Internal logging and store keys still use real `username`/UUID.

**Death recap (base)**: Enabled by default for all game modes via `buildDeathRecapTracker()` returning `DeathRecapTracker()`. Records damage with weapon (attacker's held item material) and distance (attacker-victim position distance). Displays killer info with weapon and distance, survival time since game start, per-entry weapon/distance details, and assists. `/lastdeath` command re-displays the last stored recap. Modes can override `buildDeathRecapTracker()` to return `null` to disable.

**Achievement hooks**: `GameMode` base automatically tracks achievement progress via `AchievementRegistry.progress()` at key game events: `"kills"` on kill, `"deaths"` on death, `"games_played"` on game start (for all alive players), `"wins"` on game end (for winner). Also tracks cumulative damage taken (`damageTakenCounters`) for iron_skin trigger. All calls wrapped in `runCatching` to avoid breaking game flow. Rewards (coins, XP, cosmetics) distributed automatically on unlock. Points accumulated and milestones checked on each unlock.

**ELO rating integration**: `applyRatingChanges()` runs in `enterEnding()` after `persistGameStats()`, wrapped in `runCatching` to avoid breaking game flow. Loads all player ratings from `RatingStore`, calculates per-player ELO delta via `EloCalculator.calculateDelta()` (multi-opponent, placement-weighted, survival factor approximated from placement/total ratio), applies atomically via `ApplyRatingChangeProcessor`, and sends rating change + tier promotion/demotion messages to online players. Requires `Orbit.gameMode` to be set and at least 2 players. `RatingStore` registered in Orbit's Hazelcast stores.

**Combo counter subsystem**: Optional `ComboConfig` field (`comboCounter`) managed by base. Override `buildComboCounter(): ComboConfig?` to configure. Installed in `enterPlaying()`, uninstalled in `enterWaiting()` and `shutdown()`. Uses `ComboManager.install()`/`uninstall()` from `utils/combo/`.

**Team balancing helper**: `autoBalanceTeams(teamNames: List<String>)` — protected utility for subclasses. Uses `TeamBalance.balance()` snake-draft algorithm to distribute alive players evenly across named teams. Assigns via `tracker.assignTeam()`.
...
**Spectator cycling**: `nextSpectatorTarget(player): Player?`, `previousSpectatorTarget(player): Player?` — cycles through alive players. In team mode, prioritizes teammates first, falls back to all alive. Tracks current target per spectator via `Tag.UUID("game:spectator_target")` on the Player entity.

**Spectator toolkit**: `buildSpectatorToolkit(): SpectatorToolkit?` builds the per-mode spectator overlay (see `utils/spectatortoolkit/spectatortoolkit.md`). Applied on `eliminate()`, removed on `revive()`, uninstalled on `shutdown()`. Provides a hotbar (next/previous/free-cam/player-list/speed/leave), an optional shader-driven HUD layout (`spectator-hud`: timer, alive count, watched-target health/armor/kills bars, free-cam indicator), an enriched click-to-spectate selector with health/armor/distance lore, mutual spectator hiding via `updateViewableRule`, and a free-camera mode that detaches `spectate()` and stashes the last target so toggling off resumes the same player.
...
- New players join via `tryClaimLateJoinSlot()` atomic check-and-claim (window active + max not reached, race-free).
...
- **Confirm menu**: Uses `confirmGui()` (item-based overload) with EMERALD_BLOCK confirm and REDSTONE_BLOCK cancel. Shows selected gamemode/map and cost (1 ticket). Re-validates tickets on click (fresh load from store).
...
- **Lifecycle**: Votes cast during WAITING/STARTING. Resolved in `onGameSetup()` into `votedValues: ConcurrentHashMap<String, Int>`, results broadcast to all players, then cleared.
...
- **PLAYING**:
  ...
  - Death recap: `brDeathRecapTracker` records all damage (with weapon and distance) in `onPlayerDamaged()`. On lethal hit: `sendRecap(victim)` displays damage summary with weapon/distance/survival time, then victim spectates killer for 60 ticks (kill cam). Cleared per-player on death and globally on `onGameReset()`. `install(eventNode)` for `PlayerDisconnectEvent` cleanup. Also inherits base `GameMode`'s default `deathRecapTracker`.
...
- **Nick-aware names**: All player-visible messages (elimination broadcasts, death recap attacker names, ranking reports, match result stat displays) use `Player.displayUsername` instead of `Player.username`. Internal logging and store keys use real username/UUID.
...
| Module | Summary |
|---|---|
| `crafting` | Opens crafting inventory on crafting table interaction, recipes implement Minestom Recipe API for recipe book support |
...
| File | Summary |
|---|---|
| `CustomContentRegistry.kt` | Central singleton: init, load JSON, allocate states, register items/blocks, merge pack. `@Volatile mergeResult` for thread-safe pack access. Top-level `customItem {}` and `customBlock {}` DSL functions |
...
| `block/BlockStateAllocator.kt` | Deterministic vanilla block state pools per hitbox type, `allocate(id, hitbox)` with atomic pool index via `ConcurrentHashMap.compute()`, `isAllocated(block)`, `fromVanillaBlock(block)`. Persists to `data/customcontent/allocations.dat` |
| `block/CustomBlock.kt` | Block data class (`id`, `hitbox`, `itemId`, `customModelDataId`, `hardness`, `drops`, `modelPath`, `placeSound`, `breakSound`, `allocatedState`). `item()` returns `CustomItem?` (null-safe). `CustomBlockDrops` sealed: `SelfDrop`, `LootTableDrop` |
...
| `event/CustomBlockBreakHandler.kt` | `PlayerBlockBreakEvent` listener: detects custom block → spawns drops (self-drop null-safe, loot table), plays break sound |
...
| `CustomArmorRegistry.kt` | Registry with `ConcurrentHashMap`. `loadFromResources(resources, dir)` auto-detects `.bbmodel` files. `register()` fails fast on duplicate IDs, assigns color IDs via `ModelIdRegistry`. Extension functions: `createItem(ArmorPart)`, `equipFullSet(Player)` |
...
| `modelengine/behavior/SegmentBehavior.kt` | Chain IK constraint: direction from parent, configurable angleLimit/rollLock. Manual normalization via cached inverse length to avoid redundant `sqrt`. Dot product clamped before `acos` to prevent NaN |
...
| `modelengine/vfx/VFX.kt` | Standalone ItemDisplay effect. `vfx(item) { position(); scale(); lifetime() }` DSL. Per-player show/hide. `evictViewer` sends `DestroyEntitiesPacket` before removing from viewer set. VFX IDs from `-3,800,000` |
...
| `modelengine/lod/LODHandler.kt` | Closest-viewer distance evaluation. Applies bone visibility per LOD level. Per-player culling beyond max distance. Companion registry tracks all instances; `cleanupAll(uuid)` evicts from all handlers on disconnect (wired into `ModelEngine.onPlayerDisconnect`). `dispose()` unregisters from companion and clears state |
...
| Util | Summary |
|---|---|
| `gui/Gui.kt` | `gui(title, rows) {}` DSL, paginated GUI, event-based click handling, atomic event node cleanup via `AtomicBoolean` guard (prevents double-removal on close+disconnect race). `confirmGui(title, message, onConfirm, onCancel)` for simple confirmations; `confirmGui(title, confirmItem, cancelItem, previewItem?, onConfirm, onCancel)` for custom-item confirmations. `backButton(slot, onClick)` on both `GuiBuilder` and `PaginatedGuiBuilder`. |
...
| `notification/Notification.kt` | `notify(player) { title(); message(); channels(CHAT, ACTION_BAR, TITLE, SOUND) }` DSL, `NotificationManager` broadcast/instance/player, multi-channel (CHAT, ACTION_BAR, TITLE, BOSS_BAR, SOUND), boss bar auto-cancels repeating task when progress reaches 0, `announceChat()`, `announceActionBar()`, `announceTitle()` convenience functions |
...
| `nebulaworld/NebulaWorldLoader.kt` | `NebulaWorldLoader.load(name, path)`, `loadAndPreload(name, path, centerChunkX, centerChunkZ, radius)`, `verifyLoaded(instance, pos)` (reads `.nebula` binary only; Anvil is ingest-only — see `MapLoader`) |
| `maploader/MapLoader.kt` | `MapLoader.resolve(name)` (returns `worlds/<name>.nebula` path, auto-ingests from `maps/` on first use), `scanAndIngest()` (boot-time bulk ingest of both Anvil dirs and `.nebula` files at top of `maps/`), `ingestAnvilIfPresent(name)` (copies `maps/<name>/` → `worlds/<name>/` → converts to `worlds/<name>.nebula` → deletes working copy; Anvil source preserved; skips when a valid `.nebula` already exists), `ingestNebulaIfPresent(name)` (verifies and moves `maps/<...>.nebula` → `worlds/<name>.nebula`; skips when a valid target already exists) |
| `nebulaworld/NebulaWorldConverter.kt` | `NebulaWorldConverter.convert(anvilPath, outputPath)` — ingest-time bridge, spins up a throwaway Minestom instance with `AnvilLoader` to extract sections, then serializes via `NebulaWorldWriter` (Zstd) |
...
| `blocksnapshot/BlockSnapshot.kt` | `Instance.captureSnapshot(region)`, `BlockSnapshot.capture/restore/restoreAsync/diff/pasteAt/createInstance`, `Instance.blockRestore {}` DSL with auto-restore timer, packed XYZ storage (26-bit X/Z with sign extension, 12-bit Y with sign extension), uses `AbsoluteBlockBatch` for bulk block placement |
...
| `liquidflow/LiquidFlowEngine.kt` | Queue-based liquid flow simulation. `LiquidFlowEngine(instance, sourceBlock, maxLevel, infiniteSource)` with `notifyBlockChanged(x,y,z)`, `tick()` (2048 updates/tick). Level-based spreading (0=source, 1-7=flowing, 8=falling), draining, infinite source creation (2+ adjacent sources). Used by `WaterFlowModule`. See `utils/liquidflow/liquidflow.md` |
...
| `deathrecap/DeathRecap.kt` | `DeathRecapTracker` with `recordDamage(victim, DamageEntry)`, `buildRecap(player)`, `sendRecap(player)`, `getLastRecap(uuid)`. `DamageEntry` tracks `attackerUuid`, `attackerName`, `amount`, `source`, `weapon` (Material), `distance`. `DeathRecap` includes `killerWeapon`, `killerDistance`, `survivalTimeMs`, `assists`. Enabled by default in all game modes (`buildDeathRecapTracker()` returns non-null). Enhanced display: weapon name, distance, survival time. `/lastdeath` command re-displays stored recap. Vanish-aware (hides vanished attacker names). Translation keys: `orbit.deathrecap.*` |
...
| `freeze/Freeze.kt` | `FreezeManager`, `Player.freeze()` (resets velocity to `Vec.ZERO`), `Player.isFrozen`, state via `Tag.Boolean("nebula:frozen")` |
...
| `compass/CompassTracker.kt` | `CompassTracker.track()`, action bar direction/distance, `Player.trackPlayer()`, disconnect cleanup listener. Tick task fires every 1 tick but processes each tracker only when `(tickCounter + uuid.hashCode()) % updateInterval == 0` (default `updateInterval = 10`) so per-player frequency is unchanged but cluster-wide load is distributed. |
...
| `replay/` | Dual replay system: `PacketReplayRecorder` (pixel-perfect packet capture) + `ReplayRecorder` (semantic frames). 7 frame types with binary `.nebr` format (Zstd-compressed tick chunks). `ReplayViewer` with skin-enabled fake players, death animations, POV tracking. `ReplayHighlights` detection (FIRST_BLOOD, FINAL_KILL, MULTI_KILL, LONG_RANGE_KILL, CLUTCH). `/replay` command with list/play/stop/pause/resume/speed/seek/pov/highlights/info subcommands. `ReplayStorage` supports both binary and legacy GZIP JSON via `StorageScope`. ItemHeld binary serialization writes material name. Auto-publishes `ReplayHighlightMessage` for notable highlights. See `replay/replay.md` |
...
| `damage/Damage.kt` | `DamageTracker` per-player damage history with `install(eventNode)` disconnect cleanup, `DamageIndicator` floating TextDisplay damage numbers, `DamageMultiplierManager` per-player per-source multipliers, `Player.setDamageMultiplier/getDamageMultiplier/removeDamageMultiplier` extensions |
| `raytrace/RayTrace.kt` | `rayTraceBlock()`, `rayTraceEntity()`, `raycast()` (entity+block in one pass with bounding box checks), `Player.lookDirection/rayTraceBlock/rayTraceEntity/lookingAt/getLookedAtEntity/getLookedAtBlock` extensions. Private `entityAABB()` helper deduplicates AABB min/max calculation for `raycast()` and `getLookedAtEntity()`. |
...
| `animation/Animation.kt` | `blockAnimation {}` / `entityAnimation {}` DSL, keyframes, interpolation, `packetOnly` mode for visual-only block animations via `BlockChangePacket`, non-packet block frames use `AbsoluteBlockBatch` for bulk placement |
...
| `botai/BotMovement.kt` | `object BotMovement` — shared movement helpers for all bot systems. Constants: `WALK_SPEED` (0.2158), `SPRINT_SPEED` (0.2806), `VELOCITY_SCALE` (25.0), `JUMP_IMPULSE` (8.0). Functions: `moveToward(player, target, sprint, jitter)` (normalize direction, apply Gaussian jitter offset, scale velocity, set view), `lookAt(player, target)` (eye-height yaw/pitch), `jump(player)` (ground-check + impulse). Used by BotAction, BotNavigator, FakePlayer. |
| `botai/BotNavigator.kt` | A* pathfinding + velocity-based path following for Player-type bots. `BotPathfinder.findPath(instance, start, end)` returns waypoint list. `BotPathFollower(player)` with `navigateTo/tick/stop/repath` — walk/sprint speed, jumping, dropping (1-3 blocks), water handling, stuck detection (20-tick auto-repath). Uses `BotMovement` constants and `packBlockPos` from `vanilla/BlockPos.kt`. PriorityQueue A*, 500-iteration cap. |
| `botai/BotMemory.kt` | `BotMemory` — spatial + threat memory system. Location memory (`rememberLocation/recallLocations/nearestRecalled/forgetCategory`), threat memory with damage-weighted decay (`rememberThreat/getThreatLevel/getHighestThreat/isKnownThreat`), resource memory (`rememberResource/nearestResource/forgetResource/forgetResourceAt`), player sighting tracking (`updatePlayerSighting/lastSeen/nearbyThreats`). TTL-based expiry, linear threat decay over 60s, `tick()` for cleanup. Data classes: `MemoryEntry<T>`, `ThreatEntry`, `PlayerMemoryEntry`. |
| `botai/BotPersonality.kt` | `BotPersonality(aggression, caution, resourcefulness, curiosity, teamwork)` — trait vector (0-1 per trait) that modifies goal utility scores. `BotPersonalities` object presets: `WARRIOR`, `SURVIVOR`, `EXPLORER`, `BERSERKER`, `BUILDER`, `BALANCED`, `random()`. |
| `botai/BotAction.kt` | `BotAction` interface (`start`/`tick`/`cancel` + `isComplete`) plus shared internal helpers (`blockCenter`, `blockMidpoint`, `fireInteractEvent`, `blockToMaterial`, `FOOD_MAP`). Implementations live in per-category sibling files: `MovementActions.kt` (WalkTo/SprintTo/LookAt/Wait), `CombatActions.kt` (AttackEntity/CriticalHit/ShieldBlock/ShootBow/ThrowProjectile), `MiningActions.kt` (BreakBlock/PlaceBlock/MineTunnel/MineStaircase/MineVein/PlaceTorch/DigDown), `InventoryActions.kt` (UseItem/HoldSlot/EquipItem/PickupNearbyItems/EatFood/DrinkPotion), `CraftingActions.kt` (CraftItem/SmeltItems), `ConstructionActions.kt` (BridgeForward/TowerUp/PlaceWaterBucket), `InteractActions.kt` (OpenContainer), `SequenceAction.kt` (Sequence). All movement actions read `BotAI.skillLevels[player.uuid]` for jitter. `MineVein` uses `packBlockPos`; `SmeltItems` uses `FURNACE_NAMES` from FurnaceModule. |
| `botai/BotVision.kt` | `BotVision(player)` — raycast-based perception system. Casts 32 rays every 10 ticks from player's eye position in a 120-degree FOV cone (24-block range): 16 horizontal fan, 8 downward (35deg), 4 upward (40deg), 4 random peripheral. Only 768 block lookups per scan. Transparent blocks (air, water, flowers, torches) passed through; interesting blocks (ores, logs, crafting tables, furnaces, chests) added to visible set; solid blocks stop rays. `canSee(block)`, `canSeeAny(vararg blocks)`, `visibleOres()`, `visibleEntities()` (line-of-sight verified), `canSeePlayer(target)`, `seesOpenSpace(direction)`, `visibleOfType(block)`. |
| `botai/BotExploration.kt` | `BotExploration(brain, vision)` — exploration decision system. `ExplorationInterest` sealed interface with scored interests: `VisibleOre` (0.3-0.9), `CaveOpening` (0.5), `Tree` (0.4), `Container` (0.6), `EnemyPlayer` (aggression-scaled), `DroppedItem` (0.4), `UnexploredDirection` (0.2). `findInterest()` returns highest-priority visible interest. `exploreDirection()` picks direction preferring unexplored areas (memory), open space (vision), cave following (underground). Auto-remembers seen crafting tables, furnaces, ores, logs, containers into memory. Direction changes every 200-400 ticks or on wall collision. |
| `botai/BotGoal.kt` | Abstract `BotGoal` with `calculateUtility(brain): Float` (0.0-1.0), `shouldActivate/createActions/shouldCancel`. All goals use vision-first perception. `SurviveGoal` (utility = 1-healthRatio, 0.95 if critical), `FleeGoal` (utility = hurt * proximity, uses memory threats), `AttackNearestGoal` (utility = 0.7 * proximity + weapon bonus), `EquipBestArmorGoal` (utility = unequipped * 0.15), `CraftToolGoal` (0.6 with materials, vision + memory for crafting table), `MineOreGoal` (0.55 visible / 0.4 remembered), `GatherWoodGoal` (0.4, vision for logs, exploration direction fallback), `BuildShelterGoal` (0.4 if 20+ blocks), `ExploreGoal` (0.1, uses exploration system + danger zone avoidance), `StripMineGoal` (0.45 base, descend to target Y then tunnel), `CaveExploreGoal` (0.4 base, uses exploration interest for cave detection, vision for ores), `SmeltOresGoal` (0.5, vision + memory for furnace), `ToolProgressionGoal` (0.65 if can craft better tier), `GatherResourceGoal` (0.5, vision-first ore detection), `InventoryManagementGoal` (0.55 if >80% full), `PlaceFurnaceGoal` (0.5, vision + memory for furnace check), `CriticalAttackGoal` (0.75 * proximity + 0.2 weapon bonus, requires good weapon, sprint + jump + 1.5x crit strike, boosted by aggression), `ShieldDefenseGoal` (0.8 * (1-healthRatio) when shield in offhand + nearby threats, activates ShieldBlock + Wait recovery, boosted by caution), `RangedAttackGoal` (0.75 when target > 8 blocks, falls back to melee when close, requires bow + arrows, boosted by caution), `UsePotionGoal` (0.85 when health < 6, 0.6 when health < 10, drinks healing/splash potions, boosted by caution), `BridgeGoal` (0.4 when gap detected ahead + has build blocks, measures gap length and bridges forward, boosted by resourcefulness). |
| `botai/BotSkill.kt` | `BotSkillLevel(aimAccuracy, reactionTimeTicks, criticalHitChance, blockChance, movementJitter, decisionDelay, miningEfficiency, bridgingSpeed)` — per-bot skill calibration. `BotSkillLevels` presets: `BEGINNER`, `CASUAL`, `AVERAGE`, `SKILLED`, `EXPERT`. `forRating(0.0..1.0)` linearly interpolates all fields between BEGINNER and EXPERT. Wired into BotBrain (decision delay), BotAction (aim accuracy, crit chance, shield reaction, bow spread, movement jitter). |
| `botai/BotBrain.kt` | `BotBrain(player, goals, personality, skill, memory)` — utility-scored goal selection with personality modifiers, skill calibration, and 0.1 hysteresis. Properties: `vision: BotVision`, `exploration: BotExploration`, `skill: BotSkillLevel`. `tick()` decays memories, runs vision scan, ticks exploration, updates player sightings, runs `humanize()` (random pauses, sprint toggling, look jitter), checks inventory changes (auto-organizes hotbar), scores goals every `GOAL_EVAL_INTERVAL + skill.decisionDelay` ticks. `humanize()` adds human-like imperfections: random 10-30 tick pauses (1/200), sprint drops when not in combat (1/150), random look variation (1/40). `organizeHotbar()` auto-sorts inventory on change: slot 0=sword, 1=pickaxe, 2=axe, 3=bow, 4=blocks, 5=food, off-hand=shield. `markInventoryDirty()` triggers reorganization. Inventory helpers: `hasItem/hasItemMatching/countItem/findSlot/consumeItem/giveItem/hasPickaxe`. World queries: `findNearestBlock(block)` (vision-first, then memory fallback), `findNearestEntity(type, radius)`, `findNearestPlayer(radius)`, `findNearestItem(radius)`, `findEntityByUuid(uuid)`. |
| `botai/BotLobbyFiller.kt` | `object BotLobbyFiller` — lobby fill manager for spawning/removing AI bots. `FillerConfig(targetPlayerCount, minRealPlayers, fillDelay, staggerInterval, skillRange, preset, removeOnRealJoin)`. `startFilling(gameId, gameMode, config)` waits for min real players then stagger-spawns bots with random skill/personality. `stopFilling(gameId)` removes all filler bots. `onRealPlayerJoin` removes worst-skilled bot to make room. `onRealPlayerLeave` adds a bot back during WAITING/STARTING. `getFillerBots(gameId)`, `isFillerBot(uuid)`, `getStatus(gameId)`, `activeGameIds()`. Uses `FakePlayerManager.spawn()` + `BotAI.attachPresetAI()`. |
| `botai/MiningKnowledge.kt` | `object MiningKnowledge` — Minecraft block/tool/smelting data. `requiredTool(block)`, `blockDrops(block)`, `breakTime(block, tool)`, `isOre(block)`, `smeltResult(input)` (delegates to `SMELTING_RECIPES` from FurnaceModule), `toolTier(material)` (wood=0..netherite=4), `canMine(tool, block)`, `itemValue(material)` (diamond=100..dirt=0), `materialToOre(material)`, `isPickaxe(material)`, `bestPickaxe(materials)`. Covers all vanilla ores, deepslate variants, common blocks. |
| `botai/BotAI.kt` | `object BotAI` — central registry. `install()` starts 1-tick repeating task, `uninstall()` stops + detaches all. `attach(player, vararg goals, personality, skill)`, `detach(player)`, `getBrain(player)`. All presets accept optional `skill: BotSkillLevel` parameter. `skillLevels: ConcurrentHashMap<UUID, BotSkillLevel>` stores per-player skill for action lookups. Presets: `attachSurvivalAI` (11 goals, SURVIVOR personality), `attachCombatAI` (9 goals, WARRIOR), `attachPvPAI` (10 goals, WARRIOR + BridgeGoal), `attachGathererAI` (6 goals, BUILDER), `attachPassiveAI` (2 goals, EXPLORER), `attachMinerAI` (12 goals, BUILDER). `ConcurrentHashMap<UUID, BotBrain>` storage, auto-removes offline bots. |
...
| `cinematic/CinematicCamera.kt` | `cinematic(player) { node(time, pos); lookAt(entity); loop(); onComplete {} }` DSL, keyframe path with catmull-rom/bezier/linear position interpolation, quaternion slerp rotation, dynamic lookAt target tracking, reuses `KeyframeInterpolator` + `quatSlerp`, auto-installs disconnect cleanup on first `play()` call (or explicit `install(eventNode)`) |
...
| `npcdialog/NPCDialog.kt` | `npcDialog(npcName) { page("greeting") { text(); option("quest") {} } }` DSL, tree-structured dialog, clickable chat options. `PlayerDisconnectEvent` listener cleans up conversations and per-player command handlers on disconnect. `playerCommands` map tracks commandId-to-player association for proper cleanup; old page handlers evicted on page navigation |
| `autorestart/AutoRestart.kt` | `autoRestart { after(6.hours); warnings(30.minutes, 10.minutes); warningMessage(); onRestart {} }` DSL, `AutoRestartManager.scheduleRestart/cancelRestart/getTimeRemaining`, broadcast warnings (sorted descending for correct fire order), kick on restart |
...
| `GADGET` | Usable lobby items with cooldowns | `GadgetManager` + `Cooldown<Unit>` (per-player keyed) + `PlayerUseItemEvent` |
...
- `openCosmeticList(player, category)` — paginated list, items show owned/equipped/level status, click to equip/unequip. Uses `backButton(49)` to return to category menu.
- **Unequip despawn**: when unequipping, calls `despawnCategory()` to despawn entity cosmetics (PetManager, CompanionManager, CosmeticMountManager, GadgetManager)
- **Switch despawn**: when equipping a different cosmetic in the same category, despawns the old one first via `despawnCategory()` before equipping the new one
...
### CosmeticShowcaseMenu (`CosmeticShowcaseMenu.kt`)
Read-only GUI for viewing another player's equipped cosmetics:
- `open(viewer, targetUuid, targetName)` — 5-row GUI showing target's equipped cosmetics per category
- Each category slot shows cosmetic name with rarity color, level if applicable, icon material
- Empty categories show gray glass pane with "None equipped"
- Close button in slot 40

### CosmeticCommands (`CosmeticCommands.kt`)
- `/cosmetics` — opens own cosmetic menu (existing behavior)
- `/cosmetics <player>` — opens read-only showcase menu for another player
- `/preview <cosmeticId>` — hub-only 30-second visual preview, then reverts to previous equipped state. Does not modify the store.
- `installCosmeticInteraction(handler)` — registers `PlayerEntityInteractEvent` listener: right-click another player in hub mode opens their showcase menu. Also cleans up active previews on disconnect.
...
- `updateViewers(modeled, players, ownerUuid, referencePos, maxDistance, ensureOwnerVisible)` — shared viewer management for all cosmetic managers. Iterates `players`, shows/hides based on distance to `referencePos` (<48 blocks default) and `shouldShowModel()`. When `ensureOwnerVisible = true` (default), always shows the model to its owner. Used by PetManager, CompanionManager, CosmeticMountManager, and GravestoneManager.
...
- Tick: uses `ConnectionManager.getOnlinePlayerByUuid()` for O(1) owner lookup (no map allocation). Updates position to `playerPos.add(0.8, 2.0 + sin(tick * 0.1) * 0.15, 0.0)` (hover+bob), delegates viewer management to `CosmeticVisibility.updateViewers()`, cleans disconnected players
...
- `install()` — every-tick scheduled task (stored `Task` reference). Per-player work is staggered so each player's aura still fires every `AURA_INTERVAL_TICKS = 5` ticks but the network-wide load is distributed: a player is processed only when `(tickCounter + uuid.hashCode()) % 5 == 0`.
- `uninstall()` — cancels task
- Iterates `ConnectionManager.onlinePlayers` directly (avoids nested instance iteration), checks equipped AURA cosmetic via `CosmeticDataCache.get()`, resolves level, calls `CosmeticApplier.spawnAuraParticles`
- Gated by `CosmeticListener.isAllowed(AURA, auraId)`

### PetManager (`PetManager.kt`)
Ground-following pet system using Minestom's built-in Navigator:
- `ConcurrentHashMap<UUID, ActivePet>` tracks active pets
- `install()` — 2-tick scheduled task for navigation + animation (stored `Task` reference) + `PlayerDisconnectEvent` listener via EventNode for immediate despawn
- `uninstall()` — cancels task, removes EventNode, destroys all pet models + entities
- `spawn(player, cosmeticId, level)` — creates invisible `EntityCreature` (ZOMBIE) with `MOVEMENT_SPEED` attribute set to 0.15, uses non-blocking `setInstance().thenRun {}` to attach `modeledEntity()` model after instance placement. Guards against stale entity with `creature.isRemoved` check before model creation. Reads `modelId`, `scale`, `walkAnimation` from resolved data.
- `despawn(playerId)` — destroys model + removes entity
- Tick: uses `ConnectionManager.getOnlinePlayerByUuid()` for O(1) owner lookup (no map allocation). Calculates distance to owner. If >20 blocks, reset navigator + teleport. If >3 blocks, `navigator.setPathTo(player.position)` with auto-tick movement. If close, reset navigator + idle.
...
- Viewer management: delegates to `CosmeticVisibility.updateViewers()`.
...
- `install()` — registers `PlayerDisconnectEvent` listener via EventNode for immediate unequip + cooldown cleanup
...
- `onUse(player)` — resolves cosmetic data via `CosmeticDataCache.get()`, checks per-player cooldown (keyed by `"${cosmeticId}:${player.uuid}"`), executes action
- `cleanupCooldowns(playerId)` — removes all cooldown entries for a player on disconnect
...
- `ConcurrentHashMap<UUID, ActiveGravestone>` tracks active gravestones with expiry timestamps. `ActiveGravestone` stores the spawn `Instance` reference for O(1) viewer updates (no all-instance iteration).
...
- `spawn(instance, position, cosmeticId, level, playerUuid)` — creates `standAloneModel` from resolved `modelId`, `scale`, schedules removal after `duration` seconds. Viewer management via `CosmeticVisibility.updateViewers()` with `ensureOwnerVisible = false`.
- Auto-despawns expired gravestones. If `ModelEngine.modeledEntity()` returns null (entity already cleaned up), removes the entry to prevent map leaks. Viewer management via `CosmeticVisibility.updateViewers()` respects preference changes
...
- Tick: uses `ConnectionManager.getOnlinePlayerByUuid()` for O(1) owner lookup (no map allocation), delegates viewer management to `CosmeticVisibility.updateViewers()`
...
- **Per-mode filtering**: `@Volatile var activeConfig: CosmeticConfig` — set by `Orbit.kt` from `mode.cosmeticConfig` before `install()`. `isAllowed(category, cosmeticId)` is `internal` for AuraManager/CompanionManager access. Reads `activeConfig` into a local val to avoid TOCTOU race between `enabledCategories` and `blacklist` checks.
...
- **Projectile trail**: 2-tick scheduled task — builds per-instance `entityId → Player` map for O(1) shooter lookup, passes level + ownerUuid to `spawnProjectileTrailParticle`
- **Kill effect**: `onPlayerEliminated(killer, victimPosition)` — passes level
- **Death effect**: `onPlayerDeath(player, deathPosition)` — plays death effect + spawns gravestone + despawns entity cosmetics (PetManager, CompanionManager, CosmeticMountManager)
...
- **Instance switch**: `PlayerSpawnEvent` (non-first-spawn) despawns all entity cosmetics before re-applying in the new instance
...
- 6 achievement/milestone-exclusive (price=0): `win_effect_legend`, `kill_effect_blood`, `aura_champion`, `title_achiever`, `aura_achievement`, `mount_achievement`
...
- Commands: `/cosmetics` opens own cosmetic menu, `/cosmetics <player>` opens showcase, `/preview <cosmeticId>` for temporary preview
- `installCosmeticInteraction(handler)` — right-click player in hub to view their cosmetics
...
- Unowned + price>0 → click opens `confirmGui()` (item-based overload with GREEN_WOOL confirm, RED_WOOL cancel, preview item at slot 13)
...
### Battle Pass (Seasonal) — `BattlePassRegistry.kt`, `BattlePassManager.kt`, `BattlePassMenu.kt`
- `BattlePassRegistry` — thin wrapper delegating to `BattlePassDefinitions` from Gravity (date-driven seasonal source of truth). `activePasses()`, `allPasses()`, `activePassIds()`.
- `BattlePassManager` — stateless orchestrator:
  - `addXp(player, passId, amount)` — single-pass XP addition via `AddBattlePassXpProcessor`. Applies premium XP boost (`xpBoostPremium`) before sending to processor.
  - `addXpToAll(player, amount)` — per-pass boosted XP via `AddXpToAllPassesProcessor`. Each pass gets its own boosted amount if player has premium for that pass.
  ...
  - `purchaseTier(player, passId)` — deduct `tierPurchasePrice` coins, advance 1 tier via `PurchaseTierProcessor`, rollback on max tier. Sends tier-up message + network message.
- XP sources: participation=50, per kill=10, win=100, mission completion=per-mission
- `BattlePassMenu` — three-level GUI:
  - Pass selector (3 rows): one item per active pass, shows tier + XP progress + days remaining + XP boost indicator. "Past Seasons" button (BOOK) at slot 16.
  - Tier view (6 rows, paginated): CLOCK header (slot 4) with tier/XP/total XP/countdown/boost info. 9-pane progress bar (slots 36-44) showing % to next tier. Tier items: LIME_STAINED_GLASS_PANE (claimable), GREEN_CONCRETE (claimed), RED_STAINED_GLASS_PANE (locked). Premium purchase (slot 46, GOLD_INGOT), tier purchase (slot 48, EMERALD), back (slot 49, ARROW), history (slot 50, BOOK).
  - Season history (3 rows): shows archived seasons from `seasonHistory` with final tier, completion status, total XP.
  - Premium purchase: `confirmGui()` (GREEN_WOOL confirm, RED_WOOL cancel, GOLD_INGOT preview).

### Missions — `mission/MissionRegistry.kt`, `mission/MissionTracker.kt`, `mission/MissionMenu.kt`
- `MissionRegistry` — thin wrapper delegating to `MissionTemplates` from Gravity (hardcoded source of truth, 25 daily + 15 weekly pool)
- `MissionTracker` — hooks into game events: `onKill`, `onWin`, `onGamePlayed`, `onSurvivalMinute`, `onDamageDealt`, `onTopPlacement`, `onUseCategory`, `onAssist`, `onKillStreak`, `onCraftItem`, `onMineBlock`, `onPlaceBlock`, `onEatFood`, `onWalkDistance`, `onPlayGamemode`, `onWinGamemode`, `onOpenContainer`. Each calls `IncrementMissionProcessor` → on completion grants BP XP + coins + increments `mission_master` achievement + translated notification. After any daily mission completes, calls `CheckDailyCompletionProcessor` to atomically check if all 3 dailies are done — if so, awards streak bonus (50 XP * streak, cap 350; 25 coins * streak, cap 175) and sends streak notification.
- `MissionMenu` — 4-row GUI. Slot 4 CLOCK "Daily", slots 10/12/14 daily items + slots 11/13/15 reroll buttons (BARRIER). Slot 22 COMPASS "Weekly", slots 19/21/23 weekly items. Slot 16 streak display (BLAZE_POWDER if completed today, FIRE_CHARGE otherwise) showing current streak count, bonus rewards, and status. Color-coded: LIME_DYE (completed, glowing), YELLOW_DYE (in progress + bar), GRAY_DYE (not started). Lore: reward preview + reset timer. Reroll: 50 coins, 1 per day, replaces incomplete daily mission atomically via `RerollMissionProcessor`, refunds on failure.
...
- `registerAchievementContent()` registers 33 achievements across 6 categories using `achievement {}` DSL
- Categories: GENERAL (4), COMBAT (10), SURVIVAL (7), SOCIAL (4), EXPLORATION (2), MASTERY (6)
- Each achievement has: `points` (5/15/50/100), `rarity` (COMMON/UNCOMMON/RARE/EPIC/LEGENDARY), optional `tierGroup` + `tierLevel` for visual grouping
- Tier groups: `player` (first_game/veteran/dedicated), `kill_master` (first_blood/warrior/slayer/mass_murderer), `winner` (survivor/champion/legend), `explorer` (map_explorer/world_traveler)
- Stat-based thresholds via `AchievementTriggerManager.bindThreshold()` — 19 bindings including cosmetics_owned, friends_count, total_coins_earned, total_damage_taken, food_eaten, bow_kills, unique_maps_played, team_wins
- Kill streak triggers: `double_trouble` (2), `unstoppable` (5), `rampage` (10)
- Achievement-exclusive cosmetics via `rewards` field: `mass_murderer→kill_effect_blood`, `legend→win_effect_legend`, `bp_complete→aura_champion`
- Reward distribution: automatic on unlock — coins via `AddBalanceProcessor`, XP via `BattlePassManager.addXpToAll`, cosmetics via `UnlockCosmeticProcessor`
- Milestone rewards at 50/150/300/500/750 points — tracked in `AchievementData.claimedMilestones`, claimed via `ClaimMilestoneProcessor`
- Unlock notification: toast + ENTITY_PLAYER_LEVELUP sound + chat message (name/desc/points) + reward listing + instance broadcast
- `AchievementMenu` — 5-row main view: DIAMOND summary (total points/completed), category buttons (slots 19-24) with per-category points, GOLD_INGOT milestones display (slot 25). Category view: tiered achievements grouped with bronze/silver/gold indicators, standalone achievements with rarity colors and progress bars
- Wired triggers in `BattleRoyaleMode.persistGameStats`: party_animal (PartyLookupStore check), host_master (Orbit.hostOwner check), map_explorer/world_traveler (unique map tracking), berserker (3+ kills in game), pacifist (win with 0 kills), close_call (win <2 hearts), speed_demon (win <5min), completionist (all non-hidden done), cosmetics_owned (CosmeticStore check)
- `FoodConsumptionModule` wires `MissionTracker.onEatFood` + `AchievementTriggerManager.evaluate("food_eaten")` on each food consumption
- `GameMode.installGameMechanicsNode` tracks cumulative damage taken per player via `damageTakenCounters`, evaluates `total_damage_taken` trigger for iron_skin
- `MissionTracker.checkDailyStreak` triggers `streak_master` achievement when streak >= 7

### GameMode Base Integration
- `EntityDamageEvent` listener: `MissionTracker.onDamageDealt(attacker, amount)` for PvP damage
- `eliminate()`: `MissionTracker.onTopPlacement(player)` for placements 1-3
- `enterEnding()`: `MissionTracker.onTopPlacement(winner)` for alive winners (placement 1)
- `enterPlaying()`: starts survival minute task (1200 ticks) calling `MissionTracker.onSurvivalMinute`, walk distance task (20 ticks) calling `MissionTracker.onWalkDistance`
- `handleDeath()`: `MissionTracker.onKillStreak(killer)` when streak >= 3
- `creditAssists()`: `MissionTracker.onAssist(assister)` for each assist
- `PlayerBlockBreakEvent`: `MissionTracker.onMineBlock(player)` during PLAYING
- `PlayerBlockPlaceEvent`: `MissionTracker.onPlaceBlock(player)` during PLAYING

### BattleRoyaleMode Integration
- `onPlayerDamaged`: `MissionTracker.onKill(killer)` + `MissionTracker.onDamageDealt(attacker, amount)` on lethal hits + `BattlePassManager.addXpToAll(killer, applyPartyBonus(10))` + kill streak achievement triggers
- `persistGameStats`: `MissionTracker.onGamePlayed/onWin` + `MissionTracker.onPlayGamemode/onWinGamemode("battleroyale")` + `BattlePassManager.addXpToAll` (applyPartyBonus(50) participation, applyPartyBonus(100) win) + `invincible` achievement check + `AchievementTriggerManager.evaluate` + party_animal/host_master/map_explorer/berserker/pacifist/close_call/speed_demon/completionist/cosmetics_owned triggers
- `playerMaps` — `ConcurrentHashMap<UUID, MutableSet<String>>` tracking unique maps per player per game cycle for map_explorer/world_traveler

### Party Bonus — `PartyBonusCalculator.kt`
- +5% bonus per party member in the game, capped at +20% (4 friends)
- Applies to ALL reward types: coins (via `RewardDistributor`), battle pass XP (via `applyPartyBonus` helper)
- Party memberships are **snapshotted at game start** (`enterPlaying()`) via `PartyBonusCalculator.buildPartySnapshot()` — immune to mid-game party disbands or leaves
- `GameMode.gamePartySnapshot` — `Map<UUID, Set<UUID>>` mapping each player to their party members present in the game
- `GameMode.partyBonuses` — `Map<UUID, PartyBonusCalculator.PartyBonus>` computed from the snapshot at game start
- `GameMode.applyPartyBonus(uuid, amount)` — helper that multiplies base amount by the player's party bonus multiplier (overloads for `Long` and `Double`)
- `RewardDistributor.distribute()` accepts optional `partyBonuses` map — multiplies all coin rewards by the player's bonus
- Post-game notification: `orbit.party.bonus_applied` sent to each player with bonus > 0
- Queue preview: `SelectorMenu.joinQueue()` sends `orbit.party.queue_bonus` when party has 2+ members
- Solo players or parties of 1 in the game: no bonus (multiplier = 1.0)
...
- All utility player-facing strings route through the translation system:
  ...
  - `nebulaworld/NebulaWorldLoader.kt` — kick message on unload (`orbit.util.world.unloading`)
...
| `/lastdeath` | — | — | Re-display last death recap (stored per-player in `DeathRecapTracker.lastRecaps`, cleared on game reset/disconnect) |
...
### Orbit Command (`commands/OrbitCommand.kt`)
Master admin command for the game server. Permission: `orbit.admin`. Registered in `Orbit.kt` via `commandManager.register(orbitCommand())`.

| Syntax | Description |
|---|---|
| `/orbit` | Show help / subcommand list |
| `/orbit status` | Server status: name, mode, instances, players, TPS/MSPT, memory, phase, alive count |
| `/orbit mode` | Game mode details: type, map, phase, player counts, team info, overtime/sudden death state |
| `/orbit test run <testId>` | Run a specific gametest |
| `/orbit test runall` | Run all registered gametests |
| `/orbit test cancel` | Cancel running test |
| `/orbit test list` | List available tests |
...
| `/orbit test live bots` | List spawned bots |
| `/orbit test live kill <player>` | Kill a player/bot |
...
| `/orbit test live behavior <player> <behavior>` | Set bot behavior |
| `/orbit test live behaviorall <behavior>` | Set all bots' behavior |
| `/orbit test live preset <preset> <count>` | Spawn bots with a preset |
| `/orbit test live ramp <count> [intervalTicks]` | Gradual spawn |
| `/orbit test live stress <count> <durationSec>` | Stress test with CHAOS bots |
| `/orbit test live metrics` | Show live metrics |
| `/orbit test live run <testId>` | Run scripted test against live game |
| `/orbit fill start <targetCount> [preset] [skillMin-skillMax]` | Start lobby filler: spawn bots to fill game |
| `/orbit fill stop` | Stop lobby filler and remove all filler bots |
| `/orbit fill status` | Show filler state (bot count, target, preset, skill range) |
| `/orbit reload` | Reload configuration (placeholder, no hot-reload yet) |

`/orbit status` pulls from `Orbit.serverName`, `Orbit.gameMode`, `TPSMonitor`, `MinecraftServer` instances/players, `Runtime` memory, and `GameMode` phase/tracker if active. `/orbit mode` displays full game state including team breakdown, scores, and overtime flags. `/orbit test` delegates to `GameTestRunner` and `GameTestRegistry`. `/orbit test live` manages live test sessions via `LiveTestSession`.

## Server Drain — `utils/drain/`
Phase-aware graceful drain driven by `ServerData.drain` in `ServerStore`. See `utils/drain/drain.md` for the full contract.

| File | Purpose |
|---|---|
| `HubSelector.kt` | Least-loaded active hub excluding self; falls back to any active limbo. |
| `PlayerTransfer.kt` | Cosmetic-aware transfer: despawns Pet/Companion/Mount/Gadget before publishing `TransferPlayerMessage`. `transferToHub(player)`, `transferToHubByUuid(uuid)`, `transferAllToHub()`. Used by both the drain poller and the shutdown hook. |
| `ServerDrainManager.kt` | `drain-poller` virtual-thread daemon (5s interval). Reads own `ServerData.drain` and reacts on `GameMode.phase`: hub-mode / `WAITING` / `STARTING` / `ENDING` transfer immediately; `PLAYING` waits for natural end with chat + boss bar notification every 30s and a 5-minute force-transfer timeout. Idempotent across drain toggles. |

`Orbit.kt` installs the manager after `MetricsPublisher.initialize()` and uninstalls in the shutdown hook before `MetricsPublisher.shutdown()`. The `AsyncPlayerConfigurationEvent` listener calls `ServerDrainManager.handleIncomingPlayer(uuid)` so joiners on a draining server are transferred away before `GameMode` join handling sees them — no `GameMode` modification.

Drain is set by `Gravity/.../GracefulRestartCoordinator` **before** any restart signal is issued, so by the time the shutdown hook runs the server is normally already empty. The shutdown hook is a fallback for ungraceful kills and shares the `PlayerTransfer` codepath.

## Metrics Publisher — `utils/metrics/MetricsPublisher.kt`
Publishes `ServerMetrics` and `ServiceErrors` to Hazelcast `ReplicatedMap("metrics")` every 10 seconds.

- Collects: TPS (from `TPSMonitor.averageTPS`), player count, entity count (sum across all instances), memory usage, uptime
- Keys: `Orbit.serverName` (server metrics), `errors:${Orbit.serverName}` (service errors)
- Requires `TPSMonitor.install()` to be called before initialization
- `MetricsPublisher.initialize()` — configures Ether's `ErrorAggregator` (default bounds, see Ether `observability.md`) and starts the scheduled publisher
- `MetricsPublisher.shutdown()` — stops publishing, awaits termination (5s), and removes both entries from the metrics map
- Uses `ScheduledExecutorService` with daemon thread (same pattern as `ProxyHeartbeat`)
- Errors are auto-collected by hooking `Logger.error(...)` — every Orbit log call at error level lands in the bounded aggregator

## GameTest Framework — `utils/gametest/`
In-game minigame testing framework. Tests run on the live server via `/gametest`, spawning fake players, simulating game scenarios, and reporting results to the operator in chat. Virtual-thread execution enables synchronous-style test code with `waitTicks()` and `waitUntil()`. Supports both isolated tests (new instance + GameMode) and **live mode** (attach to running game).

### Files

| File | Purpose |
|---|---|
| `GameTest.kt` | `GameTestRegistry`, `GameTestDefinition`, `GameTestBuilder`, `GameTestFixture`, `gameTest {}` DSL entry point |
| `GameTestContext.kt` | Runtime context with tick control, player simulation extensions, assertions, event assertions, metrics, world manipulation, snapshot/diff, live mode support |
| `GameTestAssertions.kt` | Fluent infix assertion DSL (`shouldBe`, `shouldContain`, `shouldHaveSize`, etc.) |
| `GameTestEdgeCases.kt` | Edge case testing: concurrency (`concurrent`, `forAllPlayers`, `stressRepeat`), timing (`atTickStart`, `atTickEnd`, `rapidFire`, `withLatency`), player state, combat, game flow, inventory edge cases, test generation helpers (`parameterizedTest`, `playerCountTests`) |
| `GameStateSnapshot.kt` | `GameStateSnapshot`, `PlayerSnapshot`, `TrackerSnapshot`, `SnapshotDiff` data classes + `capturePlayerSnapshot()`, `diffSnapshots()` top-level functions |
| `EventRecorder.kt` | `EventRecorder` + `EventCapture<E>` for recording and asserting on events during tests |
| `GameTestRunner.kt` | Orchestration: instance creation, bot spawning, lifecycle, cleanup, live session management, failure context capture |
| `GameTestCommand.kt` | `/gametest` command (run/runall/cancel/list) |
| `LiveTestSession.kt` | Live test session: attaches to running game, spawns/manages bots dynamically, behavior/preset spawning, gradual/stress tests, metrics |
| `TestBotBehavior.kt` | `TestBehavior` enum, `BehaviorConfig`, `TestBotController` — thin wrapper delegating to BotAI system. Maps TestBehavior values to BotAI goal/personality configs. Also defines `PatrolGoal` and `FollowGoal` for gametest-specific behaviors |
| `TestBotPresets.kt` | `TestBotPreset`, `TestBotPresets` — predefined bot configurations (diamond aggressive, iron defensive, archer, etc.) |
| `PacketInterceptor.kt` | `PacketInterceptor`, `TestPlayerConnection`, packet assertion extensions — intercepts outbound packets sent to test bots for verification |

### DSL
`gameTest(id) { description; playerCount; timeout; tags; withGameMode { }; useFixture(f); beforeEach { }; afterEach { }; setup { } }` — registers test in `GameTestRegistry`. `setup` block receives `GameTestContext` and runs in a virtual thread. Set `live = true` to mark a test for live mode (uses current game instead of creating new instance, `playerCount` = bots to spawn). `tags` groups tests for filtered execution via `runByTag`. `beforeEach`/`afterEach` are lifecycle hooks. `useFixture(f)` copies a `GameTestFixture`'s gameModeFactory, playerCount, timeout into the builder. `fixture { }` DSL creates reusable `GameTestFixture` instances.

### GameTestContext API
- **Tick control**: `waitTicks(n)`, `waitUntil(timeout) { condition }`, `waitForPhase(phase, timeout)`.
- **Player simulation — combat/core** (extension functions on Player): `simulateAttack(target)`, `simulateMove(pos)`, `simulateUseItem()`, `simulateBreakBlock(pos)`, `simulatePlaceBlock(pos, block)`, `simulateDamage(amount, type)`, `simulateChat(message)`, `simulateKill()`.
- **Player simulation — movement/state**: `simulateSneak(sneaking)`, `simulateSprint(sprinting)`, `simulateJump()`, `simulateLookAt(target: Point)`, `simulateLookAt(entity: Entity)`, `simulateRespawn()`.
- **Player simulation — inventory/equipment**: `simulateHeldSlot(slot)`, `simulateEquip(slot, item)`, `simulateGiveItem(item)`, `simulateDropItem()`, `simulateSwapHands()`, `simulateClearInventory()`.
- **Player simulation — block interactions**: `simulateInteractBlock(pos)` (fires `PlayerBlockInteractEvent`), `simulateOpenContainer(pos)` (invokes block handler's `onInteract`).
- **Player simulation — effects**: `simulateAddEffect(effect, duration, amplifier)`, `simulateRemoveEffect(effect)`, `simulateClearEffects()`.
- **Player simulation — health/food**: `simulateSetHealth(health)`, `simulateSetFood(food, saturation)`, `simulateHeal(amount)`.
- **Player simulation — projectiles**: `simulateShootProjectile(type, speed)` — spawns projectile entity in look direction with velocity.
- **Player simulation — utility**: `simulateCommand(command)` — executes command as player via `CommandManager`.
- **Assertions (general)**: `assert(condition, message)`, `assertEqual(expected, actual)`, `assertPhase(phase)`.
- **Assertions (tracker)**: `assertAlive(player)`, `assertDead(player)`, `assertAliveCount(n)`, `assertAliveCountAtLeast(min)`, `assertAliveCountAtMost(max)`, `assertScore(player, expected)`, `assertKills(player, expected)`, `assertDeaths(player, expected)`, `assertStreak(player, expected)`, `assertTeam(player, team)`, `assertTeamAlive(team, expected)`, `assertTeamEliminated(team)`, `assertPlacement(player, expected)`, `assertWinner(player)`.
- **Assertions (position)**: `assertPosition(player, pos, tolerance)`, `assertNear(player, target, radius)`, `assertInInstance(player, instance)`, `assertOnGround(player)`.
- **Assertions (inventory)**: `assertHasItem(player, material)`, `assertHasItemCount(player, material, count)`, `assertItemInSlot(player, slot, material)`, `assertEmptyInventory(player)`, `assertHeldItem(player, material)`.
- **Assertions (health/status)**: `assertHealth(player, expected, tolerance)`, `assertHealthAbove(player, min)`, `assertHealthBelow(player, max)`, `assertFood(player, expected)`, `assertHasEffect(player, effect)`, `assertNoEffect(player, effect)`.
- **Assertions (blocks)**: `assertBlock(pos, expected)`, `assertBlockMaterial(pos, expected)`, `assertAir(pos)`.
- **Assertions (entities)**: `assertEntityCount(type, expected)`, `assertEntityNear(pos, radius, type, minCount)`.
- **Assertions (temporal)**: `assertWithinTicks(ticks) { block }`, `assertEventually(timeout) { condition }`.
- **Fluent DSL** (`GameTestAssertions.kt`): `shouldBe`, `shouldNotBe`, `shouldNotBeNull()`, `shouldBeTrue()`, `shouldBeFalse()`, `shouldBeGreaterThan`, `shouldBeLessThan`, `shouldBeAtLeast`, `shouldBeAtMost`, `shouldBeEmpty()`, `shouldNotBeEmpty()`, `shouldContain(element)`, `shouldHaveSize(n)`.
- **Event assertions**: `assertEventFired<E>(message)`, `assertEventNotFired<E>(message)`, `assertEventCount<E>(expected, message)`, `capturedEvents<E>()`. Requires `events.record<E>()` first.
- **Packet assertions** (`PacketInterceptor.kt`, extension functions on `GameTestContext`): `assertAnyPacketSent(player, message)`, `assertPacketSent<T>(player, message)`, `assertPacketNotSent<T>(player, message)`, `assertPacketCount<T>(player, expected, message)`. `Player.receivedPackets<T>(context)` returns filtered packet list.
- **Packet helpers** (high-level game-relevant assertions): `assertReceivedMessage(player, containing)` (checks `SystemChatPacket` text via `PlainTextComponentSerializer`, case-insensitive), `assertReceivedTitle(player)` (checks `SetTitleTextPacket`), `assertReceivedActionBar(player)` (checks `ActionBarPacket`), `assertReceivedSound(player, sound)` (checks `SoundEffectPacket` by `SoundEvent`), `assertReceivedParticle(player)` (checks `ParticlePacket`).
- **Packet interceptor**: `packets` property on `GameTestContext` — `PacketInterceptor` with `packetsFor(player)`, `packetsOf<T>(player)`, `packetCount(player)`, `packetCountOf<T>(player)`, `clear(player)`, `clearAll()`, `remove(uuid)`. Auto-wired: test bots use `TestPlayerConnection` (extends `PlayerConnection`) that forwards all outbound packets to the interceptor instead of discarding them.
- **Metrics**: `collectMetrics()` returns `TestMetrics(startTps, endTps, avgTps, peakMemoryMb, memoryDeltaMb, durationMs, eventCount)`. Automatically collected and displayed on PASS.
- **Properties**: `operator`, `players` (dynamically updated in live mode), `instance`, `gameMode`, `tracker`, `phase`, `isLive`, `liveSession`, `events` (`EventRecorder`), `packets` (`PacketInterceptor`).
- **Behavior control**: `Player.setBehavior(behavior, config)`, `setAllBehavior(behavior, config)` — assign `TestBehavior` AI to bots via `TestBotController`.
- **AI control**: `Player.attachAI(preset)` (survival/combat/pvp/gatherer/passive/miner), `Player.attachAI(personality, vararg goals)` (custom), `Player.detachAI()`, `Player.brain()`, `Player.setPersonality(personality)`, `attachAllAI(preset)`, `detachAllAI()` — direct BotAI integration for fine-grained bot AI control.
- **World manipulation**: `fillBlocks(from, to, block)` fills a cuboid region, `setBlock(pos, block)` sets a single block, `platform(centerX, centerZ, radius, y, block)` creates a flat square platform, `arena(center, radiusX, radiusZ, height, wallBlock, floorBlock)` creates a walled arena with floor, `spawnItem(pos, item)` spawns an `ItemEntity`, `spawnEntity(type, pos)` spawns an `Entity`, `placeChest(pos, items)` places a chest block with inventory, `clearEntities()` removes all non-player entities from instance, `blockAt(pos)` returns the block at a position.
- **Snapshot/Diff**: `snapshot()` captures `GameStateSnapshot` (phase, tick, all player states, tracker state). `compareSnapshots(before, after)` returns `SnapshotDiff` (phaseChanged, playersJoined/Left/Died, healthChanges, killChanges, scoreChanges). `restorePlayerState(player, snapshot)` restores a player to a captured `PlayerSnapshot` (position, health, food, gameMode, inventory, equipment, effects). `withSnapshot { block }` takes before/after snapshots around a block and returns the diff. `assertChanged({ block }) { diffAssertions }` runs a block and asserts conditions on the resulting `SnapshotDiff`.
- **Live mode methods**: `spawnBots(count)`, `removeBots()` (throw `GameTestFailure` if not in live mode).
- **Edge case testing** (`GameTestEdgeCases.kt`, top-level extension functions on `GameTestContext`):
  - *Concurrency*: `concurrent(vararg actions)` (execute all on same tick), `forAllPlayers { player -> }` (action on all players same tick), `stressRepeat(times) { }` (tight loop N times).
  - *Timing*: `atTickStart { }`, `atTickEnd { }`, `rapidFire(ticks) { tick -> }` (action every tick for N ticks), `withLatency(ticks) { }` (delay block by N ticks).
  - *Player state*: `simulateDisconnectReconnect(player, disconnectTicks)`, `simulateJoinDuringPhaseTransition(phase)` (live mode only), `simulateAllDisconnect()`, `simulateRapidSpectatorSwitch(player, targets, intervalTicks)`, `forcePhase(phase)`, `simulateJoinWithFullInventory(player)`, `simulateMoveToWorldBorder(player)`, `simulateMoveToVoid(player)`.
  - *Combat*: `simulateMutualKill(player1, player2)` (both die same tick), `simulateMultiDamage(player, sources)` (multiple attackers same tick), `simulateEnvironmentDeath(player, type)`, `simulateCombatLog(player)` (record damage then disconnect), `simulateTeamKill(attacker, victim)`.
  - *Game flow*: `simulateForceStartUnderMin()`, `simulateSimultaneousElimination(players)`, `simulateTimerExpire()`, `simulateOvertimeTrigger()`, `awaitGameEnd(timeout)`, `resetAndVerify()`.
  - *Inventory*: `simulateFillInventory(player, item)`, `simulateInventoryOverflow(player, item)`, `simulatePickupItem(player, item)`.
- **Test generation helpers** (`GameTestEdgeCases.kt`): `parameterizedTest(baseId, params) { }` generates tests for each param set. `playerCountTests(baseId, counts, step) { }` generates tests for player count range.
- **Reporting**: `log(message)` sends gray message to operator.

### GameTestRunner Lifecycle
1. Creates temporary flat-world instance (bedrock + stone + grass).
2. Pre-generates bot UUIDs and installs a temporary `AsyncPlayerConfigurationEvent` interceptor (child `EventNode`) that routes test bot UUIDs to the GameMode's `activeInstance` (or flat instance if no GameMode).
3. Optionally installs GameMode on the global handler. GameMode's `install()` wraps core listeners (`PlayerSpawnEvent`, `PlayerDisconnectEvent`, `PlayerUseItemEvent`) in a child `EventNode` for clean removal.
4. Creates and installs `EventRecorder` on the global handler.
5. Spawns N fake players using `TestPlayerConnection` (wraps `PacketInterceptor`). Bots go through `doConfiguration` (intercepted to target test instance) and `transitionConfigToPlay`, triggering `PlayerSpawnEvent` with `isFirstSpawn=true` in the correct instance. All outbound packets are captured by the interceptor instead of being discarded.
6. Waits for all bots to come online (10s timeout).
7. If GameMode present, waits for all bots to be tracked by `PlayerTracker` (10s timeout).
8. Runs `beforeEach` hook (if defined).
9. Runs setup block in virtual thread with timeout watchdog.
10. Runs `afterEach` hook in finally block (even on failure).
11. Collects `TestMetrics` and reports PASS/FAIL/TIMEOUT/ERROR to operator with timing + metrics.
12. Uninstalls `EventRecorder`.
13. Cleans up: removes config interceptor node, removes bots, evicts bot `PlayerCache` entries, shuts down GameMode (removes core event node), unregisters instance.

One test per operator at a time. `runAll` executes all tests sequentially with summary. `runByTag(operator, tag)` runs all tests matching a tag.

### Failure Context Capture
When a test fails (`GameTestFailure`) or errors, the runner automatically captures a `GameStateSnapshot` and sends formatted context lines to the operator. Context includes: current phase, alive/dead player counts with names, per-player position and health, and total recorded event count. Implemented via `formatFailureContext(context)` in `GameTestRunner`.

### Live Test Sessions
`GameTestRunner.startLive(operator)` attaches to the current running `GameMode` on the operator's instance. The operator plays normally while controlling the test session. `LiveTestSession` manages bot spawning/removal, context creation, and scripted test execution against the live game. Live sessions are independent from standard tests and stored per operator UUID.

Bot spawning uses `TestPlayerConnection` (wrapping `PacketInterceptor`) + `doConfiguration` + `transitionConfigToPlay` with a temporary config interceptor routing bots to the game instance. Bots trigger `handlePlayerJoin()` naturally via `PlayerSpawnEvent`. All outbound packets are captured by the session's `PacketInterceptor`. Bot removal disconnects each bot, triggering `PlayerDisconnectEvent` naturally, evicts `PlayerCache` entries, and removes the bot's captured packets from the interceptor. Live test bots use UUID prefix `0x00FE57B0_10000000` to distinguish from standard gametest bots.

#### Bot Behavior System
`TestBotController` is a thin wrapper delegating to the BotAI system (`utils/botai/`). `setBehavior()` calls `BotAI.detach()` then `BotAI.attach()` with goals/personality mapped from the `TestBehavior` enum. `clearBehavior()` calls `BotAI.detach()`. `clearAll()` reinstalls BotAI.

`TestBehavior` enum maps to BotAI goals:
- `IDLE` — detaches AI (no goals).
- `WANDER` — `ExploreGoal` with EXPLORER personality.
- `AGGRESSIVE` — `AttackNearestGoal` + `SurviveGoal` + `EquipBestArmorGoal` with high aggression.
- `DEFENSIVE` — `FleeGoal` + `AttackNearestGoal` + `SurviveGoal` + `EquipBestArmorGoal` with high caution.
- `PATROL` — `PatrolGoal(waypoints)` (gametest-specific goal cycling through waypoints).
- `FOLLOW` — `FollowGoal(targetUuid)` (gametest-specific goal following a target player).
- `FLEE` — `FleeGoal(healthThreshold=20)` (always flee).
- `RANDOM_ACTIONS` — `SurviveGoal` + `AttackNearestGoal` + `GatherWoodGoal` + `ExploreGoal` with random personality.
- `CHAOS` — `AttackNearestGoal` + `SurviveGoal` + `ExploreGoal` with BERSERKER personality.

`BehaviorConfig` holds per-behavior parameters (targetPlayer, waypoints, attackRange, fleeHealth, wanderRadius). BotAI's 1-tick repeating task drives all behavior execution.

#### Bot Presets
`TestBotPresets` provides named preset configurations: `diamond_aggressive` (full diamond + aggressive), `iron_defensive` (full iron + defensive with 8hp flee threshold), `naked_wanderer` (no equipment + wander), `archer` (leather + bow + 16-range aggressive), `chaotic` (chaos behavior). `TestBotPreset` holds behavior, config, equipment map, health, and food. `TestBotPresets.get(name)` resolves by lowercase name; `TestBotPresets.names()` returns available names.

#### Live Mode Enhanced API
- `spawnBots(count, preset)` — spawn with preset (applies equipment, health, food, behavior).
- `gradualSpawn(count, intervalTicks, preset)` — ramp up: spawns 1 bot per interval, returns cancellable `Task`.
- `stressTest(botCount, duration)` — spawn CHAOS bots for a timed duration.
- `setBotBehavior(player, behavior, config)` — assign behavior to a single bot.
- `setAllBotBehavior(behavior, config)` — assign behavior to all session bots.
- `attachAI(player, preset)` — attach BotAI preset (survival/combat/pvp/gatherer/passive/miner) to a bot.
- `attachAllAI(preset)` — attach BotAI preset to all session bots.
- `detachAI(player)` — detach BotAI from a bot.
- `detachAllAI()` — detach BotAI from all session bots.
- `metrics()` — returns `LiveMetrics(botCount, totalPlayers, tps, memoryMb, uptimeMs)`.
- `stop()` — cancels all active tasks, detaches AI, clears behaviors, removes bots.

### Command
Gametest is accessible via `/orbit test` (permission: `orbit.admin`). The standalone `/gametest` command (permission: `nebula.gametest`) is deprecated.

| Syntax | Permission | Description |
|---|---|---|
| `/orbit test run <testId>` | `orbit.admin` | Run a specific test |
| `/orbit test runall` | `orbit.admin` | Run all registered tests |
| `/orbit test runtag <tag>` | `orbit.admin` | Run all tests with a specific tag |
| `/orbit test cancel` | `orbit.admin` | Cancel running test |
| `/orbit test list` | `orbit.admin` | List registered tests |
| `/orbit test live` | `orbit.admin` | Start live test session (attach to current game) |
| `/orbit test live stop` | `orbit.admin` | End live session, remove spawned bots |
| `/orbit test live spawn <count>` | `orbit.admin` | Spawn N bots into the current game |
| `/orbit test live fill <count>` | `orbit.admin` | Spawn bots to fill game to N players total |
| `/orbit test live bots` | `orbit.admin` | List spawned bots with alive/eliminated state |
| `/orbit test live kill <player>` | `orbit.admin` | Kill a specific player/bot |
| `/orbit test live damage <player> <amount>` | `orbit.admin` | Damage a player/bot |
| `/orbit test live phase` | `orbit.admin` | Show current game phase |
| `/orbit test live assert alive <count>` | `orbit.admin` | Assert alive player count |
| `/orbit test live assert phase <phase>` | `orbit.admin` | Assert current game phase |
| `/orbit test live behavior <player> <behavior>` | `orbit.admin` | Set bot behavior (idle/wander/aggressive/defensive/patrol/follow/flee/random_actions/chaos) |
| `/orbit test live behaviorall <behavior>` | `orbit.admin` | Set all bots' behavior |
| `/orbit test live ai <player> <preset>` | `orbit.admin` | Attach BotAI preset (survival/combat/pvp/gatherer/passive/miner) |
| `/orbit test live ai <player> off` | `orbit.admin` | Detach BotAI from bot |
| `/orbit test live ai all <preset>` | `orbit.admin` | Attach BotAI preset to all bots |
| `/orbit test live ai <player> info` | `orbit.admin` | Show bot AI status (personality, current goal) |
| `/orbit test live preset <preset> <count>` | `orbit.admin` | Spawn bots with a preset (diamond_aggressive/iron_defensive/naked_wanderer/archer/chaotic) |
| `/orbit test live ramp <count> [intervalTicks]` | `orbit.admin` | Gradual spawn: 1 bot per interval (default 20 ticks) |
| `/orbit test live stress <count> <durationSec>` | `orbit.admin` | Stress test: spawn CHAOS bots for duration |
| `/orbit test live metrics` | `orbit.admin` | Show live metrics (bots, players, TPS, memory, uptime) |
| `/orbit test live run <testId>` | `orbit.admin` | Run a scripted test against the live game |
| `/orbit test live disconnect <player>` | `orbit.admin` | Disconnect a bot from the game |
| `/orbit test live reconnect <player>` | `orbit.admin` | Force reconnect a disconnected bot (or spawn replacement) |
| `/orbit test live mutualkill <p1> <p2>` | `orbit.admin` | Both players die on the same tick |
| `/orbit test live alldisconnect` | `orbit.admin` | Eliminate all bots simultaneously |
| `/orbit test live forcestart` | `orbit.admin` | Force start game below minimum players |
| `/orbit test live forcephase <phase>` | `orbit.admin` | Force phase transition (PLAYING or ENDING) |
| `/orbit fill start <targetCount> [preset] [skillMin-skillMax]` | `orbit.admin` | Start lobby filler for current game (preset: survival/combat/pvp/miner/gatherer/passive) |
| `/orbit fill stop` | `orbit.admin` | Stop filling and remove all filler bots |
| `/orbit fill status` | `orbit.admin` | Show filler state |
| `/statue add <id> <player> [label]` | `orbit.statue` | Place a featured player statue at your position |
| `/statue remove <id>` | `orbit.statue` | Remove a statue by ID |
| `/statue list` | `orbit.statue` | List all active statues |
| `/statue refresh` | `orbit.statue` | Reload all statue data |
| `/statue move <id> <x> <y> <z>` | `orbit.statue` | Move statue to new position |
| `/statue pose <id> <animation>` | `orbit.statue` | Set statue animation (idle/wave/crossed_arms/celebrate/salute/look_around/sit) |
| `/statue leaderboard <source> <period>` | `orbit.statue` | Change leaderboard source/period for auto statues |

## Featured Player Statues — `utils/statue/`

Animated player statues for the hub displaying top players, staff, or manually selected players with real skins, equipped cosmetics, rank, and stats. Persists non-auto-managed statues to `statues.json`.

### `StatueManager` (`utils/statue/StatueManager.kt`)
Core lifecycle manager. `ConcurrentHashMap<String, ActiveStatue>` storage. Two rendering backends: ModelEngine (preferred) using a single reusable `player_statue` blueprint with per-statue skins resolved from GameProfile UUID, NPC (fallback) with packet-based fake player.
- **`install()`** — registers the `player_statue` blueprint via `PlayerModelGenerator.registerBlueprint()`, starts 1-tick animation loop, loads saved configs from `statues.json`.
- **`uninstall()`** — cancels tick task + auto-refresh task, removes all statues.
- **`spawn(id, config)`** — checks ban status via `SanctionStore` (skips banned players), loads player data from `PlayerStore`, `RankManager`, `CosmeticStore`, `LevelStore`, `RatingStore`. First attempts ModelEngine backend via `trySpawnModel()`, falls back to NPC via `spawnNpc()`. Creates hologram, spawns pet/companion cosmetics, shows to all instance players. Returns `ActiveStatue?`. Saves configs after spawn.
- **`trySpawnModel(config, skin)`** — checks if `player_statue` blueprint is registered. Spawns hidden skin NPC + `StandaloneModelOwner` with auto-playing idle animation. Returns `null` on failure (triggers NPC fallback).
- **`remove(id)`** — destroys NPC/model owner + hologram + pet entity + companion model. Saves configs after removal.
- **`moveStatue(id, newPos)`** — removes and respawns statue at new position.
- **`setAnimation(id, animationName)`** — stops all animations and plays the specified one on the ModelEngine model.
- **`setLeaderboardConfig(source, period)`** — changes the leaderboard source/period used for auto-managed statue resolution.
- **`refreshTopPlayerStatues(instance, podiums)`** — smooth refresh: compares new top players vs current, only removes/respawns if the player changed for a position. Updates hologram text if same player (stats may have changed).
- **`installAutoRefresh(instance, podiums)`** — initial spawn + 5-minute periodic refresh of top-player statues.
- **`spawnPetCompanion(statue, cosmeticData)`** — checks PET/COMPANION equipped cosmetics, spawns pet `EntityCreature` with ModelEngine model near the statue, spawns companion `StandaloneModelOwner` offset from statue.
- **Click handler** — plays `ENTITY_EXPERIENCE_ORB_PICKUP` sound (0.5 vol, 1.2 pitch), plays "wave" animation on ModelEngine statues (reverts to idle after 20 ticks), opens `StatueProfileMenu`.
- **Persistence** — `saveConfigs()` writes non-auto-managed statues to `statues.json` via `GsonProvider.pretty`. `loadConfigs()` reads on install via `GsonProvider.default`. `SavedStatueConfig` data class.
- **Tick loop**: distance culling (64-block radius), rotation (smooth yaw per tick, NPC only), cosmetic caching (30s TTL), aura particles (every 5 ticks), trail particles (every 20 ticks), effect particles (kill/win/spawn every 100 ticks), rank particles (#1 flame, #2 end_rod, #3 composter every 10 ticks in 1.5-block radius circle).

### `StatueConfig`
Fields: `playerUuid`, `position`, `instance`, `rotationSpeed`, `showCosmetics`, `showHologram`, `label`, `leaderboardSource` (default `"rating:battleroyale"`), `leaderboardPeriod` (default `"ALL_TIME"`), `autoManaged` (default `false`).

### `ActiveStatue`
Fields: `id`, `config`, `npc?`, `modelOwner?`, `hologram?`, `playerName`, `petEntity?`, `petModeled?`, `companionOwner?`, `cachedCosmetics?`, `cosmeticCacheTime`, `skinFetchTime`.

### `PlayerModelGenerator` (`utils/statue/PlayerModelGenerator.kt`)
Public API surface (62 lines) for the player statue blueprint. Implementation is split across three sibling files in the same package:
- `PlayerModelMesh.kt` (`internal fun buildPlayerModel(): BlockbenchModel`) — generates the cube/bone/UV hierarchy.
- `PlayerModelAnimations.kt` (`internal fun buildAnimations()` + `rotKeyframe`/`posKeyframe` helpers) — keyframe authoring.
- `BlockbenchSerializer.kt` (`internal fun exportBbmodelJson(model)` + serialize helpers + `Vec.toJsonArray`/`FloatArray.toJsonArray`) — JSON round-trip.

Generates ONE `BlockbenchModel` (`player_statue`) with exact Minecraft player proportions, standard 64x64 skin UV mapping (including all 6 overlay layers: hat, jacket, sleeves, pants), and bone hierarchy for animations. Supports file-first loading: if `models/player_statue.bbmodel` exists in the data directory, loads from file; otherwise generates programmatically and auto-exports the `.bbmodel` for designer editing in Blockbench.
- **`BLUEPRINT_NAME`** — constant `"player_statue"`.
- **`registerBlueprint()`** — loads from `models/player_statue.bbmodel` if present, falls back to programmatic generation + auto-exports the file. Called once at startup via `StatueManager.install()`. No-op if already registered.
- **`isRegistered()`** — checks if `player_statue` blueprint exists in `ModelEngine`.
- **`exportBbmodelJson(model)`** (in `BlockbenchSerializer.kt`) — serializes a `BlockbenchModel` into standard Blockbench JSON format, round-trip compatible with `BlockbenchParser.parse()`.
- **`exportToFile(outputPath)`** — builds the player model and writes the `.bbmodel` JSON to the given path.
- **File-first loading flow**: first boot generates + exports; designer edits in Blockbench; next boot loads from file; corrupted/deleted file falls back to re-generation.
- **Bone hierarchy**: `root` -> `head`, `body`, `right_arm`, `left_arm`, `right_leg`, `left_leg`. Each bone contains base cube + overlay cube (hat/jacket/sleeves/pants) with correct UV mapping.
- **Animations**: `idle` (2s loop, body sway + head nod), `wave` (1s once, right arm -120 deg), `crossed_arms` (hold, both arms crossed), `celebrate` (1.5s once, both arms raised -150 deg + body Y offset), `salute` (1.5s once, right arm -45 deg), `look_around` (4s loop, head rotates left 40 deg -> right 40 deg), `sit` (hold, legs -90 deg X, body shifted down).
- **Skin resolution**: no skin download needed -- client resolves skin from GameProfile UUID via Mojang skin servers. Resource pack contains ONE model, zero additional pack size per statue.

### `StatueProfileMenu` (`utils/statue/StatueProfileMenu.kt`)
6-row GUI opened on statue click. Async data load via virtual thread.
- Row 0: Player head with name, rank, level, total wins/kills/games/win rate.
- Row 2: Equipped cosmetics (one slot per equipped category, up to 7).
- Row 4: Rating tiers per gamemode (tier color, display name, peak rating, games played).

### `StatueCommand` (`utils/statue/StatueCommand.kt`)
`/statue` command (permission: `orbit.statue`). Subcommands: `add`, `remove`, `list`, `refresh`, `move`, `pose`, `leaderboard`. Player UUID resolved via `PlayerStore` indexed predicate query on `name`.

### Wiring
- `HubMode.install()` calls `StatueManager.install()`.
- `HubMode.shutdown()` calls `StatueManager.uninstall()` first.
