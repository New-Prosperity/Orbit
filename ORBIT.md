...
- Common listeners: `AsyncPlayerConfigurationEvent` (set spawning instance/respawn from mode), `PlayerSpawnEvent` (locale tag via `LOCALE_TAG` on Player entity, trace ID tag via `TRACE_TAG` from `SessionStore`, nick application + nicked-player broadcast to joining viewer), `PlayerDisconnectEvent` (cleanup), `OnlinePlayerCache` refresh (5s). Locale is stored as `Tag.String("nebula:locale")` on the Player entity — no manual eviction needed, auto-cleans on entity removal. Trace ID stored as `Tag.String("nebula:traceId")` — read from `SessionData.traceId` (set by Nova on login), fallback to first 8 chars of player UUID. Extension property `Player.traceId` provides convenient access. `localeOf(UUID)` resolves locale by looking up the player from the connection manager.
...
Single reusable player model blueprint for the ModelEngine backend. Generates ONE `BlockbenchModel` (`player_statue`) with exact Minecraft player proportions, standard 64x64 skin UV mapping (including all 6 overlay layers: hat, jacket, sleeves, pants), and bone hierarchy for animations. Supports file-first loading: if `models/player_statue.bbmodel` exists in the data directory, loads from file; otherwise generates programmatically and auto-exports the `.bbmodel` for designer editing in Blockbench.
