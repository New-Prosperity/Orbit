...
- `GameSettings(worldPath, preloadRadius, spawn, scoreboard, tabList, lobby, hotbar, timing, cosmetics, teams, respawn, lateJoin, overtime, mapName)` — wraps all config types. Optional fields default to `null` (disabled). `cosmetics` defaults to `CosmeticConfig()` (all enabled).
...
- `PlayerTracker` — `ConcurrentHashMap<UUID, PlayerState>` with full team, lives, and stats tracking.
  ...
  - **State methods**: `join`, `eliminate`, `revive`, `markRespawning`, `disconnect`, `reconnect`, `remove`, `stateOf`, `isAlive`, `isSpectating`, `isDisconnected`, `isRespawning`, `isActive`, `contains`, `clear`.
...
- `CosmeticDefinition` data class: `id`, `category`, `nameKey`, `descriptionKey`, `rarity`, `material`, `data`, `maxLevel` (default 1), `levelOverrides` (default empty)
- `resolveData(level)` — merges base data with level overrides up to given level
...
- `install()` — 1-tick scheduled task for position updates
...
- Tick: updates position to `playerPos.add(0.8, 2.0 + sin(tick * 0.1) * 0.15, 0.0)` (hover+bob), manages viewers within 48 blocks filtered by `CosmeticVisibility.shouldShowModel()`, hides viewers who changed preference to REDUCED/NONE, cleans disconnected players
...
- `install()` — 5-tick scheduled task
- Iterates all instance players, checks equipped AURA cosmetic, resolves level, calls `CosmeticApplier.spawnAuraParticles`
...
- `install()` — 2-tick scheduled task for pathfinding + movement
- `spawn(player, cosmeticId, level)` — creates invisible `EntityCreature` (ZOMBIE) with `modeledEntity()` model attached. Reads `modelId`, `scale`, `walkAnimation` from resolved data.
- `despawn(playerId)` — destroys model + removes entity
- Tick: calculates distance to owner. If >20 blocks, teleport. If >3 blocks, A* pathfind via `Pathfinder.findPath()` and move along path at 0.15 speed. If close, idle.
...
- `onUse(player)` — resolves cosmetic data, checks `Cooldown<UUID>` per gadget type, executes action
...
- `install()` — 20-tick scheduled task for expiry check + viewer management
...
- `install()` — installs `MountManager` + 1-tick scheduled task for viewer management + animation
- `spawn(player, cosmeticId, level)` — creates invisible `EntityCreature` (ZOMBIE) with `modeledEntity()`, adds `MountBehavior` to seat bone at runtime, gives saddle item in slot 8
...
JSON array of `CosmeticDefinition` objects. Bundled default has 27 cosmetics across 15 categories:
- `armor_knight` (ARMOR_SKIN/RARE)
- `kill_flame_burst` (KILL_EFFECT/COMMON), `kill_heart_explosion` (KILL_EFFECT/EPIC)
- `trail_flame` (TRAIL/COMMON, maxLevel 3 with level overrides), `trail_soul` (TRAIL/RARE), `trail_enchant` (TRAIL/EPIC)
...
- `companion_mini_dragon` (COMPANION/LEGENDARY, maxLevel 3)
- `spawn_lightning` (SPAWN_EFFECT/EPIC)
- `death_soul_release` (DEATH_EFFECT/RARE)
- `aura_flame` (AURA/COMMON, maxLevel 5)
- `elim_msg_royal` (ELIMINATION_MESSAGE/EPIC)
- `pet_wolf` (PET/RARE, maxLevel 3), `pet_fox` (PET/EPIC)
- `join_quit_royal` (JOIN_QUIT_MESSAGE/EPIC), `join_quit_shadow` (JOIN_QUIT_MESSAGE/LEGENDARY)
- `gadget_firework_launcher` (GADGET/COMMON), `gadget_paint_blaster` (GADGET/RARE), `gadget_grappling_hook` (GADGET/EPIC, maxLevel 3)
- `gravestone_cross` (GRAVESTONE/RARE), `gravestone_angel` (GRAVESTONE/LEGENDARY, maxLevel 3)
- `mount_horse` (MOUNT/RARE, maxLevel 3), `mount_dragon` (MOUNT/LEGENDARY)
