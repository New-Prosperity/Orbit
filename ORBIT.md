...
| `botai/BotAction.kt` | Sealed interface of atomic bot actions. All movement actions read `BotAI.skillLevels[player.uuid]` for jitter. `WalkTo` (movement jitter from skill), `SprintTo` (movement jitter from skill), `BreakBlock` (hardness-based break time, drop spawning), `PlaceBlock`, `AttackEntity` (approach with jitter + aim accuracy check — miss = swing but no damage), `UseItem`, `HoldSlot`, `EquipItem`, `PickupNearbyItems`, `OpenContainer`, `CraftItem` (material consumption + result), `EatFood` (32-tick eating, health restore), `Wait`, `LookAt`, `Sequence` (ordered action chain), `MineTunnel` (2-high tunnel with periodic torches), `MineStaircase` (descending staircase), `MineVein` (BFS flood-fill ore mining, max 32 blocks, uses `packBlockPos`), `SmeltItems` (walk to furnace, consume input+fuel, produce output, uses `FURNACE_NAMES` from FurnaceModule), `PlaceTorch` (place torch if available), `DigDown` (safe staircase-pattern descent), `CriticalHit` (sprint toward target with jitter, jump, wait for peak via velocity check, strike with 1.5x or 1.0x based on `criticalHitChance` from skill), `ShieldBlock` (reaction delay from `skill.reactionTimeTicks` before activating off-hand shield, hold for durationTicks, visual-only blocking), `ShootBow` (equip bow, aim offset proportional to `(1-aimAccuracy)*4.0` blocks Gaussian spread, draw animation via metadata, release after chargeTicks), `ThrowProjectile` (equip projectile item, look at target, fire `PlayerUseItemEvent`), `PlaceWaterBucket` (place water block, swap to empty bucket), `BridgeForward` (walk forward placing blocks under feet across gaps, consumes block material), `TowerUp` (jump + place block under self repeatedly, jump-peak detection), `DrinkPotion` (find potion in inventory, fire `PlayerUseItemEvent`). Each action has `start/tick/cancel` + `isComplete`. Delegates movement to `BotMovement`. `FOOD_MATERIALS` set, `blockToMaterial` helper. |
...
- `MetricsPublisher.shutdown()` — stops publishing, awaits termination (5s), and removes the entry from the metrics map
