...
object BattleRoyaleVoteManager {
    ...
    fun resolve(categoryId: String): Int {
        val cat = season.voteCategories.firstOrNull { it.id == categoryId } ?: return 0
            ...
            categories.forEachIndexed { i, cat ->
                ...
                slot(slotIndex, itemStack(material) {
                    ...
                    if (current != null) {
                        ...
                        opt?.let {
                            lore(player.translateRaw("orbit.game.br.vote.current", "option" to player.translateRaw(it.nameKey)))
                        }
                    }
        ...
        gui(player.translateRaw(cat.nameKey), rows = 3) {
            ...
            cat.options.forEachIndexed { i, option ->
                ...
                slot(slotIndex, itemStack(material) {
                    ...
                    if (isSelected) {
                        lore(player.translateRaw("orbit.game.br.vote.selected"))
                        glowing()
                    } else {
