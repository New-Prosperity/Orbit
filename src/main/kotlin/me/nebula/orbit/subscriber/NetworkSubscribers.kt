...
import me.nebula.gravity.messaging.ConfigUpdateMessage
...
fun installNetworkSubscribers(gameMode: String?) {
    NetworkMessenger.subscribe<PlayerReportMessage> { msg ->
        for (p in MinecraftServer.getConnectionManager().onlinePlayers) {
            ...
            if ("*" in perms || "staff.reports" in perms) {
                p.sendMessage(p.translate(Keys.Orbit.Report.StaffAlert,
    ...
    NetworkMessenger.subscribe<PartyInviteMessage> { msg ->
        ...
        invitee.showToast {
            title(invitee.translateRaw(Keys.Orbit.Party.InviteSubtitle, "player" to msg.inviterName))
    ...
    NetworkMessenger.subscribe<FriendOnlineMessage> { msg ->
        ...
        for (friendId in msg.friendIds) {
            ...
            p.sendMessage(p.translate(Keys.Orbit.Friend.Online, "player" to msg.playerName))
            p.playSound(friendOnlineSound)
        }
    }

    if (gameMode != null) {
        NetworkMessenger.subscribe<ConfigUpdateMessage> { msg ->
            if (msg.entryKey.startsWith("MAINTENANCE_") && msg.newValue == "true") {
                val disabledMode = msg.entryKey.removePrefix("MAINTENANCE_").lowercase()
